  ...
  config = mkMerge [
    ...
    (mkIf (pkgs.stdenv.isLinux && cfg.enable) {
      ...
      systemd.user.services.virtualbox-suspend-inhibitor = {
        ...
        serviceConfig = {
          ExecStart = ''
            ${lib.getExe pkgs.bash}/bin/bash -c " \
              while true; do \
                # Wait until a VM is running \
                while ! ${lib.getExe' pkgs.virtualbox "VBoxManager"} list runningvms | ${lib.getExe pkgs.gnugrep}/bin/grep -q '.'; do \
                  sleep 30; \
                done; \
                \
                ...
                ${lib.getExe pkgs.bash}/bin/bash -c 'while ${lib.getExe' pkgs.virtualbox "VBoxManager"} list runningvms | ${lib.getExe pkgs.gnugrep}/bin/grep -q \".\"; do sleep 30; done'; \
                \
