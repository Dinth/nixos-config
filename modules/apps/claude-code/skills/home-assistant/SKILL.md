...
description: >
  ...
  (config share mounted at /mnt/haos). Use when creating/editing automations,
  scripts, scenes, helpers, templates, dashboards, or ESPHome configs; debugging
  an automation; or changing anything on the HA instance. Covers the local
  MCP-first/verify workflow — NOT the generic HA rules, which live in the
  homeassistant MCP's own home-assistant-best-practices skill.
---
...
`home-assistant-best-practices` skill. For deep delegation use the
`home-assistant` subagent; full detail is in `~/.claude/CLAUDE.md`.
...
> That skill targets the **official built-in HA MCP**, which is less capable than
> ours. Our `homeassistant` server is a superset — follow its principles but reach
> for our richer `ha_*` toolset; don't assume a capability is missing.

## 2. Work through the `homeassistant` MCP (write-capable)
The MCP is wired at `/mnt/haos` and `/mnt/haos/esphome` (CIFS mount of the live
HAOS `/config`).
...
## 5. Escalation
Need cross-host log correlation or historical search? Ask the `triage-specialist`
agent (it owns the Grafana MCP).
